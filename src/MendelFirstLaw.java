...
*/
    public static void main(String[] args) {
        System.out.println(processMendelFirstLaw("26 29 21")); // 0.783859649122807
    }
    ...
    public static double processMendelFirstLaw(String input) {
        String[] inputArray = input.split(" ");
        int k = Integer.parseInt(inputArray[0]); // homozygous dominant
        int m = Integer.parseInt(inputArray[1]); // heterozygous
        int n = Integer.parseInt(inputArray[2]); // homozygous recessive
        // Total number of organisms
        int total = k + m + n;
        double probability = 0;
        double kProbability = k / (double) total;
        double mProbability = m / (double) total;
        double nProbability = n / (double) total;
        // Probability of k mating with k
        probability += kProbability * (k - 1) / (double) (total - 1);
        // Probability of k mating with m
        probability += kProbability * m / (double) (total - 1);
        // Probability of k mating with n
        probability += kProbability * n / (double) (total - 1);
        // Probability of m mating with k
        probability += mProbability * k / (double) (total - 1);
        // Probability of m mating with m
        probability += mProbability * (m - 1) / (double) (total - 1) * 0.75;
        // Probability of m mating with n
        probability += mProbability * n / (double) (total - 1) * 0.5;
        // Probability of n mating with k
        probability += nProbability * k / (double) (total - 1);
        // Probability of n mating with m
        probability += nProbability * m / (double) (total - 1) * 0.5;
        // Probability of n mating with n (no dominant allele) = 0
        probability += nProbability * (n - 1) / (double) (total - 1) * 0;
        return probability;
    }
